/*
This is version 3 of my coursework deadline program.
This has full functionality, but does not compensate for:

- February (Awkward AF)
- Leap Years
- Assignments 12months or further away

and will most-likely not as none of these factors affect me.
*/
...
void order_work(char *courseName[], char *assignmentName[], char *dueDate[], char *dueTime[], int entries);
...
int main(void) {
	...
	for(int i = 0; i < noOfLines; i++) {
		courseName[i] = calloc(sizeof(char), BUFFER_SIZE);
		assignmentName[i] = calloc(sizeof(char), BUFFER_SIZE);
		dueDate[i] = calloc(sizeof(char), BUFFER_SIZE);
		dueTime[i] = calloc(sizeof(char), BUFFER_SIZE);
	}
	...
	/* Calculates the remaining days - dueDate now holds the number of days remaining*/
	calc_days_left(dueDate, noOfLines);

	/* Organises into order of due date */
	order_work(courseName, assignmentName, dueDate, dueTime, noOfLines);
...
void order_work(char *courseName[], char *assignmentName[], char *daysLeft[], char *dueTime[], int entries) {

	int smallest;
	for(int i = 0; i < entries; i++) {

		/* Base case */
		if(courseName[i] == NULL || !strcmp(courseName[i], " "))
			return;

		smallest = i;
		for(int j = i + 1; j < entries; j++) {
			
			int currentDaysLeft = atoi(daysLeft[j]);
			int smallestsDaysLeft = atoi(daysLeft[smallest]);
			int lessDays = currentDaysLeft < smallestsDaysLeft;
			int sameDays = currentDaysLeft == smallestsDaysLeft;

			//printf("CHour:%s \nSHour:%s\n", dueTime[j], dueTime[smallest]);


			/* About tempDue
				This pointer is needed as strtok replaces the token with '\0'
				in the ORIGINAL string.
			*/

			char *tempDueTime = malloc(6 * sizeof(char));

			strcpy(tempDueTime, dueTime[j]);
			int currentHour = atoi(strtok(tempDueTime, ":"));
			int currentMinute = atoi(strtok(NULL, ":"));
			
			strcpy(tempDueTime, dueTime[smallest]);
			int smallestsHour = atoi(strtok(tempDueTime, ":"));
			int smallestsMinute = atoi(strtok(NULL, ":"));
			int lessTime = 0;


			if(currentHour < smallestsHour) //If it's a previous hour
				lessTime = 1;	
			else if ((currentHour == smallestsHour) && (currentMinute < smallestsMinute)) //If it's previous minute in same hour
				lessTime = 1;

			if((lessDays) || (sameDays && lessTime)) 
				smallest = j;

			free(tempDueTime);
		}

		/* If the smallest has changed */
		if(smallest != i) {

			char *temp[PARAMETERS];

			/* Copy of smallest in temp */
			temp[0] = courseName[smallest];
			temp[1] = assignmentName[smallest];
			temp[2] = daysLeft[smallest];
			temp[3] = dueTime[smallest];

			/* Moving position i into smallest's position */
			courseName[smallest] = courseName[i];
			assignmentName[smallest] = assignmentName[i];
			daysLeft[smallest] = daysLeft[i];
			dueTime[smallest] = dueTime[i];

			/* Moving Temp into i*/
			courseName[i] = temp[0];
			assignmentName[i] = temp[1];
			daysLeft[i] = temp[2];
			dueTime[i] = temp[3];
		}
